...
class MaskDINODecoder(nn.Module):
    ...
    def prepare_for_dn(self, targets, tgt, refpoint_emb, batch_size):
        ...
        if self.training:
            ...
            # print(f"known_labels max: {known_labels.max().item()}, known_labels min: {known_labels.min().item()}")
            ...
            # print(f"m min: {m.min().item()}, m max: {m.max().item()}, num_classes: {self.num_classes}")
            assert m.min() >= 0 and m.max() < self.num_classes, "Label index out of range!"
            # print(f"m dtype: {m.dtype}, expected: torch.int64")
            assert m.dtype == torch.int64, "Labels should be of type torch.int64 (long)!"
